...
def make_tri_mfs(width, clist):
    ...
    return [TriangularMembFunc(c-width/2, c, c+width/2) if c-width/2 > 0 else TriangularMembFunc(c, c, c+width/2) for c in clist]
...
def make_anfis(x, num_mfs, num_out, hybrid=True):
    '''
    ...
    for i in range(num_invars):
        sigma = ranges[i] / num_mfs[i]
        mulist = torch.linspace(0, maxvals[i], num_mfs[i]).tolist()
...
def make_anfis_T(x, num_mfs, num_out, hybrid=True, dataframe=pd.DataFrame):
    '''
        Make an ANFIS model, auto-calculating the (Gaussian) MFs.
        I need the x-vals to calculate a range and spread for the MFs.
        Variables get named x0, x1, x2,... and y0, y1, y2 etc.
    ...
    i = 0
    while i < num_invars: #prova con dicretizzazione ottimizzata e con equal width discretizer
        clist = (val for val in dataframe.iloc[:, i].unique())
        invars.append(('x{}'.format(i), make_tri_mfs(ranges[i], clist)))
